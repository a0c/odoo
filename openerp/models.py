    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False, lazy=True):
        ...
        aggregated_fields = [
            ...
            if self._fields[f].type in ('integer', 'float') or hasattr(self._fields[f], 'group_operator') and self._fields[f].group_operator
    ...
    def _read_from_database(self, field_names, inherited_field_names=[]):
        ...
        if missing:
            ...
            exc = AccessError(
                _('The requested operation cannot be completed due to security restrictions. Please contact your system administrator.\n\n(Document type: %s, Operation: %s, IDs: %s, User: [#%s] %s)') % \
                (self._name, 'read', missing.ids, self.env.user.id, self.env.user.name)
            )
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        if field_name and not isinstance(field_name, list) and \
                not self._context.get('allow_2many_update_while_modify') and \
